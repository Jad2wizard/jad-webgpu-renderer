	...
	constructor(paths: PathProps[], style?: Style) {
		...
		for (let p of paths) {
			...
			if (!!p.startTime && pathStyle.headPointVisible) {
				this.headPointList.push(new HeadPoint(pathModel, pathStyle as Required<Style>))
			}
		}
	}
			...
			if (pathModel) {
				if ('headPointVisible' in _style) {
					...
					if (!_style.headPointVisible && existedHeadPointIndex !== -1) {
						...
						_style.headPointVisible &&
						existedHeadPointIndex === -1 &&
						pathModel.startTimes
					) {
						this.headPointList.push(new HeadPoint(pathModel, _style as Required<Style>))
					}
				}
