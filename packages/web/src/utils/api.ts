...
export function updateLayer(
  ...
  return api<{ layer: Record<string, unknown> }>(
...
export function fetchChatMessages(sessionId: string) {
  return api<{ messages: { id: string; role: string; content: string; toolCalls: string; createdAt: string }[] }>(`/chat/sessions/${sessionId}`)
}
