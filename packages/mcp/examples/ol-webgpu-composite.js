	...
	await page.evaluate(
		({ tileUrl, centerLon, centerLat, zoom }) => {
			...
			tileSource.on('tileloadstart', () => {
				/* @ts-ignore */ window.__tilesLoading++
			})
			tileSource.on('tileloadend', () => {
				/* @ts-ignore */ window.__tilesLoading--
				/* @ts-ignore */ window.__tilesLoadedCount++
			})
			...
			map.once('rendercomplete', () => {
				/* @ts-ignore */ window.__renderComplete = true
			})
			map.updateSize()
			map.renderSync()
		},
	...
	await page.evaluate(
		({ width, height, count, pointSize }) => {
			...
			const run = async () => {
				...
				if (!context) {
					throw new Error('无法获取 WebGPU CanvasContext')
				}
			...
			run().catch((e) => {
				console.error('WebGPU 渲染失败:', e)
				/* @ts-ignore */ window.__scatterRendered = false
			})
		},
		{ width, height, count: scatterCount, pointSize }
	)

	await page.waitForFunction(() => window.__scatterRendered === true, {
		timeout: 20000,
		polling: 200,
	})
